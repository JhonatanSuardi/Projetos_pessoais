...
def controle(i,j):
    ...
    PORT = 5005            # Porta que o T1 usa controle
    
    #Criando o servidor Controlador
    controle = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    orig = (HOST, PORT)
    controle.bind(orig)
    controle.listen(128)
...
while True:
    ...
    for i in range (ni):
      ...
      # reserva
      HOSTS = '127.0.0.1'     # Endereco IP do T1
      PORTS = 5004            # Porta que o T1 esta
